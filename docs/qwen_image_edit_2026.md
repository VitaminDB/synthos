...
совпадают): скачивались один раз, в `Qwen-Image-Edit/` лежали жёсткими ссылками
на файлы `Qwen-Image-Edit-2511/`. SHA-256 всех весов сверены с `lfs.oid` HF,
бандлы — с каталогами (2856 тензоров байт в байт), `syn-verify` — ок, прогон нод
шёл из `.syn`; после этого исходные каталоги удалены (освободилось 93 ГБ),
скачивать заново — только с HF. Квант NVFP4/MXFP8 делается при загрузке DiT.
