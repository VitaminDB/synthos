...
pub struct MsgAttachment {
    ...
    #[serde(default)]
    pub has_thumb: bool,
    /// Передать модели путь к файлу на диске рядом с его содержимым
    /// (галочка в полосе вложений). Хранится на вложении, а не читается из
    /// настроек при сборке промпта: текст старого сообщения обязан быть
    /// одинаковым от хода к ходу, иначе префикс-KV обнуляется.
    #[serde(default)]
    pub share_path: bool,
}
