...
use super::super::super::types::{DataBlob, QwenImageBlob, FluxModelHandle, NodeInstance, NodeRuntime, PortValue};
...
impl NodeExecutor for CheckpointExec {
    fn evaluate(&self, ctx: &mut EvalContext<'_>) {
        ...
        let pv = match ctx.runtime().lock() {
            Ok(g) => match &*g {
                NodeRuntime::QwenImageCheckpoint { model_path, device_idx, quant_idx, memory_mode_idx, resident, handle_cache } => {
                    ...
                    match handle {
                        Some(h) => {
                            ...
                            PortValue::Data(Arc::new(DataBlob::QwenImage(QwenImageBlob::Model(arc))))
                        }
...
pub fn body(node: &NodeInstance) -> Box<dyn Widget> {
    ...
    let rows: Vec<Box<dyn Widget>> = vec![
        field_row(
            ...
            node_file_picker(tr!("node.qwen_image_checkpoint.model_tooltip"), model_path, SYN_FILTER, |_| {}),
        ),
        ...
        Box::new(Text::new(tr!("node.qwen_image_checkpoint.hint")).class("flux-node-info")),
    ];
