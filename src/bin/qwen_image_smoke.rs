//! Сквозной прогон нод Qwen-Image без GUI: те же `on_run`/`evaluate`, что
//! жмёт кнопка Run, — Checkpoint → Image → Reference [→ Reference] → Text
//! Encoder (промпт + картинки) → Sampler (+ картинки, [FLUX Empty Latent]) →
//! VAE Decode → Image Save.
//!
//! ```sh
//! cargo run --release --bin qwen_image_smoke -- <qwen-image-edit*.syn|каталог> <картинка> <out.png> [steps [width height]]
//! ```
//! env: QWEN_PROMPT, QWEN_IMAGE2=<вторая картинка> (2509/2511), QWEN_SEED,
//! QWEN_CFG (4; 1 — без CFG), QWEN_NEGATIVE (по умолчанию " "),
//! QWEN_QUANT=nvfp4|mxfp8|dense, QWEN_MEMORY_MODE=auto|resident|block_offload,
//! QWEN_RESIDENT=1, QWEN_REPEAT=N (повторы: кэш промпта и удержанный DiT),
//! QWEN_VRAM_GB=7 — балласт в VRAM (модели остаётся столько гигабайт).
...
use synthos::pages::node_editor::nodes::{flux, image, qwen_image};
...
fn status(ctx: &NodeEditorCtx, id: NodeId) -> String {
    match &*node(ctx, id).runtime.lock().unwrap() {
        NodeRuntime::QwenImageSampler { loaded_name, .. }
        | NodeRuntime::QwenImageTextEncoder { loaded_name, .. }
        | NodeRuntime::QwenImageReference { loaded_name, .. } => loaded_name.get_untracked().unwrap_or_default(),
        _ => String::new(),
    }
}
...
fn run() -> Result<(), String> {
    ...
    if args.len() < 4 {
        return Err(format!(
            "использование: {} <qwen-image-edit*.syn|каталог> <картинка> <out.png> [steps [width height]]",
            args[0]
        ));
    }
    let model_path = PathBuf::from(&args[1]);
    let first = PathBuf::from(&args[2]);
    let out = PathBuf::from(&args[3]);
    let steps: u32 = args.get(4).and_then(|s| s.parse().ok()).unwrap_or(0);
    let size: Option<(u32, u32)> = match (args.get(5), args.get(6)) {
        (Some(w), Some(h)) => Some((w.parse().map_err(|_| "width")?, h.parse().map_err(|_| "height")?)),
        _ => None,
    };
    let env = |k: &str| std::env::var(k).ok().filter(|s| !s.is_empty());
    let prompt = env("QWEN_PROMPT").unwrap_or_else(|| {
        "Replace the sky with a starry night and make the lighthouse light glow bright blue; keep the cliff and the waves unchanged"
            .into()
    });
    let seed: u64 = env("QWEN_SEED").and_then(|s| s.parse().ok()).unwrap_or(42);
    let cfg: f32 = env("QWEN_CFG").and_then(|s| s.parse().ok()).unwrap_or(4.0);
    let negative = std::env::var("QWEN_NEGATIVE").ok();
    let repeat: usize = env("QWEN_REPEAT").and_then(|s| s.parse().ok()).unwrap_or(1).max(1);
    let images: Vec<PathBuf> = std::iter::once(first).chain(env("QWEN_IMAGE2").map(PathBuf::from)).collect();

    flux::shared::ensure_kernels_registered();
    let _ballast = env("QWEN_VRAM_GB").and_then(|v| v.parse::<f64>().ok()).map(|gb| {
    ...
    let n_ckpt = ctx.add_node(NodeKind::QwenImageCheckpoint, zero);
    let n_prompt = ctx.add_node(NodeKind::TextView, zero);
    let n_enc = ctx.add_node(NodeKind::QwenImageTextEncoder, zero);
    let n_smp = ctx.add_node(NodeKind::QwenImageSampler, zero);
    let n_dec = ctx.add_node(NodeKind::QwenImageVaeDecode, zero);
    ...
    for (i, src) in images.iter().enumerate() {
        ...
        let n_ref = ctx.add_node(NodeKind::QwenImageReference, zero);
    ...
    let last = *refs.last().expect("хотя бы одна картинка");
    ...
    if let NodeRuntime::QwenImageCheckpoint { model_path: mp, quant_idx, memory_mode_idx, resident, .. } =
        &*node(&ctx, n_ckpt).runtime.lock().unwrap()
    {
        mp.set(Some(model_path.clone()));
        if let Some(q) = env("QWEN_QUANT") {
        ...
        if let Some(m) = env("QWEN_MEMORY_MODE") {
        ...
        resident.set(env("QWEN_RESIDENT").is_some_and(|v| v == "1"));
    }
    set_text(&ctx, n_prompt, &prompt);
    if let NodeRuntime::QwenImageSampler { steps: st, seed: sd, cfg: c, .. } = &*node(&ctx, n_smp).runtime.lock().unwrap() {
        st.set(steps);
        sd.set(seed);
        c.set(cfg);
    }
    ...
    eprintln!(
        "qwen_image_smoke: {} | картинок {} | {} | шаги {steps} (0 — по модели) cfg {cfg} seed {seed}",
        model_path.display(),
        images.len(),
        size.map(|(w, h)| format!("{w}×{h}")).unwrap_or_else(|| "размер с картинки".into()),
    );
    for round in 1..=repeat {
        eprintln!("— прогон {round}/{repeat}");
        for (i, r) in refs.iter().enumerate() {
            run_node(&ctx, *r, &format!("reference-{}", i + 1), qwen_image::reference::on_run, &mut min_free)?;
            eprintln!("    {}", status(&ctx, *r));
        }
        let enc = run_node(&ctx, n_enc, "text-encoder", qwen_image::text_encoder::on_run, &mut min_free)?;
        eprintln!("    {}", status(&ctx, n_enc));
        let smp = run_node(&ctx, n_smp, "sampler", qwen_image::sampler::on_run, &mut min_free)?;
        eprintln!("    {}", status(&ctx, n_smp));
        run_node(&ctx, n_dec, "vae-decode", qwen_image::vae::on_run, &mut min_free)?;
        run_node(&ctx, n_save, "image-save", image::save_on_run, &mut min_free)?;
        eprintln!("  итого: энкодер {enc:.1}s, сэмплер {smp:.1}s");
        if let NodeRuntime::QwenImageSampler { seed: sd, .. } = &*node(&ctx, n_smp).runtime.lock().unwrap() {
...
fn main() {
    ...
    if let Err(e) = run() {
        eprintln!("qwen_image_smoke: ошибка: {e}");
        std::process::exit(1);
    }
}
