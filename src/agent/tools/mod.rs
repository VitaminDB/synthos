...
//! - [`autotools`]  — пул инструментов по запросу: схемы не объявляются,
//!   модель подгружает нужную ответом `autotools` и зовёт инструмент по имени.
//!
...
//!   `autoskill` и `autotools`: скил и схема не выхлоп, а инструкция, и уходят
//!   модели целиком (потолок [`executor::MAX_SKILL_OUTPUT_BYTES`], укладке не
//!   подлежат).
