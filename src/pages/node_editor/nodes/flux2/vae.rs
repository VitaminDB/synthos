//! FLUX.2 VAE: Encode (картинка → латент для img2img) и Decode (латент
//! сэмплера → картинка). VAE (~330 МБ в F32) грузится на время стадии.
...
use super::super::flux::vae::auto_size;
...
impl NodeExecutor for VaeEncodeExec {
    fn evaluate(&self, ctx: &mut EvalContext<'_>) {
        ...
        let pv = match ctx.runtime().lock() {
            Ok(g) => match &*g {
                NodeRuntime::Flux2VaeEncode { out, output_version, .. } => {
                    if track {
...
pub fn encode_on_run(node: &NodeInstance, ctx: &NodeEditorCtx) {
    let snapshot = match node.runtime.lock() {
        Ok(g) => match &*g {
            NodeRuntime::Flux2VaeEncode { resize_idx, running, error, out, output_version } => {
    ...
    running.set(true);
    error.set(None);
    let _ = thread::Builder::new().name("synthos-flux2-vae-encode".into()).spawn(move || {
        ...
        log_worker_done("flux2-vae-encode", started, &res);
        match res {
            Ok(l) => {
...
pub fn encode_busy_signal(node: &NodeInstance) -> Option<RwSignal<bool>> {
    match node.runtime.lock() {
        Ok(g) => match &*g {
            NodeRuntime::Flux2VaeEncode { running, .. } => Some(*running),
            _ => None,
        },
        Err(_) => None,
    }
...
pub fn encode_body(node: &NodeInstance) -> Box<dyn Widget> {
    let snapshot = match node.runtime.lock() {
        Ok(g) => match &*g {
            NodeRuntime::Flux2VaeEncode { resize_idx, running, error, .. } => Some((*resize_idx, *running, *error)),
            _ => None,
        },
        Err(_) => None,
    };
    ...
    Box::new(Column::new().gap(3.0).cross_axis_alignment(CrossAxisAlignment::Stretch).children(vec![
        ...
        Box::new(Text::new(tr!("node.flux2_vae_encode.hint")).class("flux-node-info")),
        field_row(
