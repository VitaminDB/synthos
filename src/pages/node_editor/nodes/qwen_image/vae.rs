//! Qwen-Image VAE Decode: латент сэмплера → картинка. VAE (~250 МБ) грузится
//! на время стадии и считается в F32.
...
impl NodeExecutor for VaeDecodeExec {
    fn evaluate(&self, ctx: &mut EvalContext<'_>) {
        ...
        let pv = match ctx.runtime().lock() {
            Ok(g) => match &*g {
                NodeRuntime::QwenImageVaeDecode { out, output_version, .. } => {
                    if track {
...
pub fn on_run(node: &NodeInstance, ctx: &NodeEditorCtx) {
    let snapshot = match node.runtime.lock() {
        Ok(g) => match &*g {
            NodeRuntime::QwenImageVaeDecode { running, error, out, output_version } => {
    ...
    let Some(latent) = current_input_latent(ctx, node.id, "latent").filter(|l| l.tensor.is_some()) else {
        error.set(Some(tr!("node.qwen_image_vae_decode.connect_latent")));
        return;
    };

    running.set(true);
    error.set(None);
    let _ = thread::Builder::new().name("synthos-qwen-image-vae-decode".into()).spawn(move || {
        let started = log_worker_start("qwen-image-vae-decode", &format!("{}x{}", latent.width, latent.height));
        let res = worker(&handle, &latent);
        log_worker_done("qwen-image-vae-decode", started, &res);
        match res {
            Ok(img) => {
...
fn worker(handle: &FluxModelHandle, latent: &FluxLatent) -> Result<ImageData, String> {
    let Some(t) = &latent.tensor else {
        return Err(tr!("node.qwen_image_vae_decode.connect_latent"));
    };
...
pub fn busy_signal(node: &NodeInstance) -> Option<RwSignal<bool>> {
    match node.runtime.lock() {
        Ok(g) => match &*g {
            NodeRuntime::QwenImageVaeDecode { running, .. } => Some(*running),
            _ => None,
        },
        Err(_) => None,
    }
