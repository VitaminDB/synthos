...
/* ─────────────── Левая группа тулбара: скрепка + hint ─────────────── */

/* Row без `flex-grow` меряется по натуральной ширине детей (движок даёт
 * нефлексовым детям `max_width = INFINITY`), поэтому длинная строка
 * ошибки раздувала левую группу и выталкивала правую — «Продолжить»,
 * regen, «Отправить» — за правый край панели: нажать их было нельзя.
 * `flex-grow: 1` отдаёт левой группе ровно остаток ширины после правой. */
.input-toolbar-left {
    flex-grow: 1;
}

/* Hint внутри левой группы — тоже растяжимый: только так он получает
 * конечный max-width, и Text переносит строку вместо одной бесконечной.
 * Число строк ограничено в коде (`max_lines`), дальше — многоточие. */
.input-hint {
    flex-grow: 1;
    background-color: transparent;
    color: var(--text-muted);
...
/* Цвет тут не задаём: он наследуется от обёртки, иначе явное правило
 * ребёнка перебило бы `.input-hint.error` ниже. */
.input-hint-text {
    font-size: 13px;
}

/* Ошибка — тревожным цветом. */
.input-hint.error {
    color: var(--error);
}
