...
//! - `bash` запускается через `bash -lc "<cmd>"`; вывод обрезается до
//!   [`executor::MAX_OUTPUT_BYTES`], чтобы не раздуть контекст. Исключение —
//!   `autoskill`: скил не выхлоп, а инструкция, и уходит модели целиком
//!   (потолок [`executor::MAX_SKILL_OUTPUT_BYTES`], копия для промпта не
