...
/*
 * overflow: hidden — обязателен. Пустое состояние («Откройте файл из
 * дерева слева») центрируется по своим bounds, и при узкой верхней
 * половине вертикального SplitView'а (терминал поднят высоко) его блок
 * выше самой панели: без клипа текст рисовался поверх tab-bar'а и
 * содержимого терминала.
 */
