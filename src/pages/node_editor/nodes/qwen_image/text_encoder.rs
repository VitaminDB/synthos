//! Qwen-Image Text Encoder: промпт вместе с картинками правки (вход
//! `references`, те же, что идут в сэмплер) → скрытые состояния Qwen2.5-VL.
//! Негатив для true CFG — со входа `negative`, без него — `" "`, как в
//! примерах карточек моделей; кодируется с теми же картинками.
...
use synaptix_image_qwen::QwenConditioning;
...
use super::super::super::types::{DataBlob, FluxModelHandle, NodeInstance, NodeRuntime, PortValue, QwenImageBlob, QwenImageRefs};
...
/// Негатив по умолчанию (примеры в карточках Qwen-Image-Edit и 2511).
...
impl NodeExecutor for TextEncoderExec {
    fn evaluate(&self, ctx: &mut EvalContext<'_>) {
        ...
        let pv = match ctx.runtime().lock() {
            Ok(g) => match &*g {
                NodeRuntime::QwenImageTextEncoder { out, output_version, .. } => {
                    if track {
                        let _ = output_version.get();
                    }
                    match out.lock().ok().and_then(|g| g.clone()) {
                        Some(c) => PortValue::Data(Arc::new(DataBlob::QwenImage(QwenImageBlob::Conditioning(c)))),
...
pub fn on_run(node: &NodeInstance, ctx: &NodeEditorCtx) {
    let snapshot = match node.runtime.lock() {
        Ok(g) => match &*g {
            NodeRuntime::QwenImageTextEncoder { running, error, loaded_name, out, output_version } => {
    ...
    let negative = current_input_text(ctx, node.id, "negative").unwrap_or_else(|| DEFAULT_NEGATIVE.to_string());
    ...
    let _ = thread::Builder::new().name("synthos-qwen-image-encode".into()).spawn(move || {
        let started = log_worker_start(
            "qwen-image-text-encoder",
            &format!(
                "промпт {} симв., картинок {}",
                prompt.chars().count(),
                refs.as_ref().map(|r| r.images.len()).unwrap_or(0)
            ),
        );
        let res = worker(&handle, &prompt, &negative, refs.as_deref());
        log_worker_done("qwen-image-text-encoder", started, &res.as_ref().map(|(_, n)| n.clone()));
        match res {
            Ok((cond, name)) => {
...
fn worker(
...
) -> Result<(Arc<QwenConditioning>, String), String> {
    let model = shared::load_model(handle)?;
    let m = &model.model;
    let images: Vec<Tensor> = refs.map(|r| r.images.iter().map(|i| i.tensor.clone()).collect()).unwrap_or_default();
    if m.variant().is_edit() && images.is_empty() {
        return Err(tr!("node.qwen_image_text_encoder.connect_references"));
    }
    let keys: Vec<String> = refs.map(|r| r.images.iter().map(|i| i.key.clone()).collect()).unwrap_or_default();
    // Пустой негатив — без CFG (вдвое быстрее), как `negative_prompt=None`.
    let neg = (!negative.is_empty()).then_some(negative);
    ...
    let dev = m.device();
    let cond = shared::with_vram_retry(dev, || m.encode_prompt(prompt, neg, &images).map_err(|e| e.to_string()))?;
    shared::trim_pool(dev);
    let tokens = cond.embeds.dims()[1];
    let cond = Arc::new(cond);
    shared::remember_conditioning(key, cond.clone());
    Ok((cond, tr!("node.qwen_image_text_encoder.done", tokens = tokens, images = images.len())))
}

pub fn busy_signal(node: &NodeInstance) -> Option<RwSignal<bool>> {
    match node.runtime.lock() {
        Ok(g) => match &*g {
            NodeRuntime::QwenImageTextEncoder { running, .. } => Some(*running),
            _ => None,
        },
        Err(_) => None,
    }
...
pub fn body(node: &NodeInstance) -> Box<dyn Widget> {
    let snapshot = match node.runtime.lock() {
        Ok(g) => match &*g {
            NodeRuntime::QwenImageTextEncoder { running, error, loaded_name, .. } => {
    ...
    Box::new(Column::new().gap(3.0).cross_axis_alignment(CrossAxisAlignment::Stretch).children(vec![
        Box::new(Text::new(tr!("node.qwen_image_text_encoder.hint")).class("flux-node-info")),
        field_row(
