...
pub struct SynChatCtx {
    ...
    /// Agent-loop остановился так, что ему стоит дать ещё ходов: упёрся в
    /// `MAX_AGENT_TURNS`, зациклился (guard повторов), ушёл целиком в
    /// reasoning — или закончил ход текстом, не вызвав при активных
    /// инструментах ни одного (модель объявила намерение и остановилась).
    /// Взводится воркером, гасится при любом новом запуске генерации. UI
    /// показывает по нему кнопку «Продолжить»: история цела, надо лишь дать
    /// циклу ещё бюджет ходов.
