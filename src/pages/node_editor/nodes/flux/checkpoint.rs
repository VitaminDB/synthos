                ...
                NodeRuntime::FluxCheckpoint {
                    ...
                    match handle {
                        Some(h) => {
                            ...
                            PortValue::Data(Arc::new(DataBlob::Flux(FluxBlob::Model(arc))))
                        }
...
pub fn body(node: &NodeInstance) -> Box<dyn Widget> {
    ...
    let rows: Vec<Box<dyn Widget>> = vec![
        field_row(
            ...
            node_file_picker(tr!("node.flux_checkpoint.model_tooltip"), model_path, SYN_FILTER, |_| {}),
        ),
        ...
        // Трансформер остаётся в VRAM между прогонами.
