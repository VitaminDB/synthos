...
/* Оба габарита заданы явно. Раньше стояли только min-width/min-height, и
 * карточка растягивалась по самому широкому ребёнку — одна длинная строка
 * статуса (текст ошибки загрузки модели) раздувала её до ~1300px. Без
 * явной высоты `measure_column` берёт `constraints.max_height`, то есть
 * всю высоту вьюпорта, и карточка вытягивалась на весь экран.
 *
 * 640×860 под содержимое: аура 360 + статус + две секции транскрипта одна
 * под другой (`panel.rs::transcripts_view`) + ряд действий. */
.voice-overlay-card {
