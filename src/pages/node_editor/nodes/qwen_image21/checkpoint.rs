...
use super::super::super::types::{DataBlob, NodeInstance, NodeRuntime, PortValue, QwenImage21Blob, QwenImage21ModelHandle};
use super::super::acestep::{field_row, make_toggle};
use super::{DEVICE_OPTIONS, MEMORY_MODE_OPTIONS, QUANT_OPTIONS, RESOLUTION_OPTIONS};
                ...
                NodeRuntime::QwenImage21Checkpoint {
                    ...
                    match handle {
                        Some(h) => {
                            ...
                            PortValue::Data(Arc::new(DataBlob::QwenImage21(QwenImage21Blob::Model(arc))))
                        }
    ...
    let snapshot = match node.runtime.lock() {
        Ok(g) => match &*g {
            NodeRuntime::QwenImage21Checkpoint {
                ...
                resident,
                ..
            } => Some((*model_path, *device_idx, *quant_idx, *memory_mode_idx, *resolution_idx, *resident)),
            _ => None,
        },
        Err(_) => None,
    };
    let Some((model_path, device_idx, quant_idx, memory_mode_idx, resolution_idx, resident)) = snapshot else {
        return Box::new(Column::new());
    };
    let rows: Vec<Box<dyn Widget>> = vec![
        field_row(
            &tr!("nodes.common.model"),
            node_file_picker(tr!("node.qwen_image21_checkpoint.model_tooltip"), model_path, SYN_FILTER, |_| {}),
        ),
        ...
        field_row(&tr!("node.qwen_image21_checkpoint.resolution"), node_dropdown_field(RESOLUTION_OPTIONS, resolution_idx)),
        // DiT остаётся в VRAM между прогонами.
        field_row(&tr!("nodes.common.keep_in_memory"), make_toggle(resident)),
        Box::new(Text::new(tr!("node.qwen_image21_checkpoint.hint")).class("flux-node-info")),
    ];
