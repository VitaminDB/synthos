...
use super::{DEVICE_OPTIONS, QUANT_OPTIONS};
...
impl NodeExecutor for CheckpointExec {
    fn evaluate(&self, ctx: &mut EvalContext<'_>) {
        ...
        let pv = match ctx.runtime().lock() {
            Ok(g) => match &*g {
                NodeRuntime::SdxlCheckpoint { model_path, device_idx, quant_idx, resident, handle_cache } => {
                    ...
                    match handle {
                        Some(h) => {
                            ...
                            PortValue::Data(Arc::new(DataBlob::Sdxl(SdxlBlob::Model(arc))))
                        }
    ...
    let snapshot = match node.runtime.lock() {
        Ok(g) => match &*g {
            NodeRuntime::SdxlCheckpoint { model_path, device_idx, quant_idx, resident, .. } => {
                Some((*model_path, *device_idx, *quant_idx, *resident))
            }
            _ => None,
        },
        Err(_) => None,
    };
    let Some((model_path, device_idx, quant_idx, resident)) = snapshot else {
        return Box::new(Column::new());
    };
    let rows: Vec<Box<dyn Widget>> = vec![
        field_row(
            &tr!("nodes.common.model"),
            node_file_picker(tr!("node.sdxl_checkpoint.model_tooltip"), model_path, SYN_FILTER, |_| {}),
        ),
        ...
        // UNet остаётся в VRAM между прогонами.
        field_row(&tr!("nodes.common.keep_in_memory"), make_toggle(resident)),
        Box::new(Text::new(tr!("node.sdxl_checkpoint.hint")).class("flux-node-info")),
    ];
