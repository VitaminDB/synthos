...
//! - [`transcribe`] — `(model, audio) → score`: запись → партитура ABC
//!   (SheetSage2) — мелодия для кавера или полная партитура с аккордами.
//!
//! Один бандл `yue2-3b.syn` держит обе ветки модели: AR (партитура и музыка) и
//! NAR (акустика), а третьим компонентом — транскриптор SheetSage2. Они
//! грузятся по отдельности и в панели «Модели в памяти» видны отдельно — NAR
//! нужен только на стадии акустики, SheetSage2 — только в начале кавера.
